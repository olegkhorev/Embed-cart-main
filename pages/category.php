...
if ($category['cleanurl']) {
	$template["navigation_script"] = $category['cleanurl']."?sort=".$_GET['sort'].'&direction='.$_GET['direction'].'&';
	$template["sort_by_script"] = $category['cleanurl']."?";
} else {
	$template["navigation_script"] = $categoryid."?sort=".$_GET['sort'].'&direction='.$_GET['direction'].'&';
	$template["sort_by_script"] = $categoryid."?";
}
...
if ($_GET['filter']) {
	...
	if ($category['cleanurl']) {
		$template["navigation_script"] = $category['cleanurl']."?sort=".$_GET['sort'].'&direction='.$_GET['direction'].$url_add.'&';
		$template["sort_by_script"] = $category['cleanurl']."?".$url_add.'&';
	} else {
		$template["navigation_script"] = $categoryid."?sort=".$_GET['sort'].'&direction='.$_GET['direction'].$url_add.'&';
		$template["sort_by_script"] = $categoryid."?".$url_add.'&';
	}
}
