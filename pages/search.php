...
$template['bread_crumbs'][] = array('', lng('Search'));
...
if ($_GET['q']) {
	$template["navigation_script"] = "search?q=".$_GET['q']."&sort=".$_GET['sort'].'&direction='.$_GET['direction'];
	$template["sort_by_script"] = "search?q=".$_GET['q'].'&';
} else {
	$template["navigation_script"] = "search?sort=".$_GET['sort'].'&direction='.$_GET['direction'];
	$template["sort_by_script"] = "search?";
}
...
if ($_GET['filter']) {
	...
	$template["navigation_script"] = "search?sort=".$_GET['sort'].'&direction='.$_GET['direction'].$url_add.'&';
	$template["sort_by_script"] = "search?".$url_add.'&';
}
...
$template['head_title'] = 'Результаты поиска. '.$template['head_title'];
