	...
	if ($action == 'featured_products') {
	    if ($mode == 'update') {
	        if (is_array($posted_data)) {
    	        foreach ($posted_data as $productid => $v) {
    	            ...
    	            $db->array2update("featured_products", $query_data, "productid='$productid' AND categoryid='".$get['2']."'");
        	    }
	        }
    	} elseif ($mode == 'delete' && !empty($to_delete)) {
			foreach ($to_delete as $productid => $v)
				$db->query("DELETE FROM featured_products WHERE productid='$productid' AND categoryid='$get[2]'");
	        ...
	        $db->query("REPLACE INTO featured_products (productid, orderby, enabled, categoryid) VALUES ('$newproductid','$neworderby','$newenabled', '$get[2]')");
	    }
		...
		if ($mode == 'delete' && $cat)
			...
			foreach ($posted_data as $k=>$v) {
				...
				$db->array2update("categories", $v, "categoryid='".$k."'");
			}
		}
...
if ($get['2']) {
	...
	$category_location = '<a href="'.$current_location.'/admin/categories/" class="ec_ajax_link">'.lng('Categories').'</a>';
	$tmp = array();
	while ($parent = $db->row("SELECT * FROM categories WHERE categoryid='".$parentid."'")) {
		$parentid = $parent['parentid'];
		$tmp[] = ' &gt; <a href="'.$current_location.'/admin/categories/'.$parent['categoryid'].'" class="ec_ajax_link">'.$parent['title'].'</a>';
	}
