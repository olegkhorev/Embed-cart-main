		...
		if ($_FILES) {
			...
			if (!empty($images)) {
				...
				foreach ($images as $image) {
					echo '<div data-id="'.$image['photoid'].'"><img src="'.$current_location.'/images/close.gif" class="remove" alt="" />';
					$image['new_width'] = 100;
		...
		if ($get['2'] != 'add') {
			...
			if ($active_modules['Product_Options']) {
				...
				if ($is_variants) {
					...
					if ($has_variants)
						$html .= '<li'.(($get['3'] == 'variant_images') ? ' class="active"' : '' ). '><a href="'.$url.'/variant_images">'.lng('Variants Images').'</a></li>';
				}
			}
