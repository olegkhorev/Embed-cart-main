...
if ($get['2']) {
	...
	$template['location'] .= ' &gt; <a href="'.$current_location.'/admin/categories">'.lng('Categories').'</a> &gt; '.$category['title'];
	$template['head_title'] = lng('Category').' '.$category['title'].' :: '.$template['head_title'];
	$category_location .= ' &gt; <a href="'.$current_location.'/admin/categories/'.$category['categoryid'].'" class="ec_ajax_link">'.$category['title'].'</a>';
