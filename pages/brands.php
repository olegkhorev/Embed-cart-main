...
if (!empty($get['1'])) {
	...
	if ($brand['cleanurl']) {
		$template["navigation_script"] = "brands/".$brand['cleanurl']."?sort=".$_GET['sort'].'&direction='.$_GET['direction'].'&';
		$template["sort_by_script"] = "brands/".$brand['cleanurl']."?";
	} else {
		$template["navigation_script"] = "brands/".$brand['brandid']."?sort=".$_GET['sort'].'&direction='.$_GET['direction'].'&';
		$template["sort_by_script"] = "brands/".$brand['brandid']."?";
	}
...
if ($_GET['filter']) {
	...
	if ($brand['cleanurl']) {
		$template["navigation_script"] = "brands/".$brand['cleanurl']."?sort=".$_GET['sort'].'&direction='.$_GET['direction'].$url_add.'&';
		$template["sort_by_script"] = "brands/".$brand['cleanurl']."?".$url_add.'&';
	} else {
		$template["navigation_script"] = "brands/".$brand['brandid']."?sort=".$_GET['sort'].'&direction='.$_GET['direction'].$url_add.'&';
		$template["sort_by_script"] = "brands/".$brand['brandid']."?".$url_add.'&';
	}
}
	$template['products'] = $products;
	$template['per_row'] = 4;
	$template['products_results_html'] = get_template_contents('common/products_results.php');
	if (!$_GET['original'] && $is_ajax && ($_GET['page'] || $is_sort || $_GET['filtered'])) {
	...
	if ($total_items > 0) {
		...
		$template["navigation_script"] = "brands/?";
		...
		foreach ($brands as $k=>$v) {
			$brands[$k]['products_count'] = $db->field("SELECT COUNT(productid) FROM products WHERE brandid='".$v['brandid']."' AND status='1'");
		}
#echo '<pre>';
#exit(print_R($brands));
