...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	$pass = array('pswd', 'new_pswd', 'password', 'descr', 'fulldescr', 'content', 'message', 'data');
			...
			foreach ($v as $k2=>$v2) {
				if (is_array($v2)) {
					foreach ($v2 as $k3=>$v3) {
						if (is_array($v3)) {
							foreach ($v3 as $k4=>$v4)
								$_POST[$k][$k2][$k3][$k4] = htmlspecialchars($v4);
						} else
							$_POST[$k][$k2][$k3] = htmlspecialchars($v3);
					}
				} else
					$_POST[$k][$k2] = htmlspecialchars($v2);
			}
		} else
...
if ($get['0'] != 'admin' && $_SESSION['current_language']) {
...
$template['parnet_site_page'] = $parnet_site_page;
$template['parent_site_location'] = $parent_site_location;
...
if ($_POST['login'] || $_GET['login'] || $_POST['userinfo'] || $_GET['userinfo'])
	redirect('/');
...
if ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest' || $get['0'] == 'ajax' || $_GET['its_ajax_page']) {
