...
echo '<script src="'.$current_location.'/var/cache/'.$lng.'/js.js?'.$css_js_cache.'"></script>';

$already = array();
foreach ($js as $v) {
	if (!$already[$v]) {
		if (!in_array($v, $default_js))
			echo '<script async src="'.$current_location.'/var/cache/'.$lng.'/js/'.$v.'.js?'.$css_js_cache.'"></script>';
