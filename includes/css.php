...
echo '@import url("'.$current_location.'/var/cache/css.css?'.$css_js_cache.'");';
$already = array();
foreach ($css as $v) {
	if (!$already[$v]) {
		if ($v == 'overflow') {
			if ($custom_css) {
				echo '@import url("'.$current_location.'/'.$custom_css.'");';
			}
		}

		if (!in_array($v, $default_css))
			echo '@import url("'.$current_location.'/var/cache/other/css/'.$v.'.css?'.$css_js_cache.'");';
