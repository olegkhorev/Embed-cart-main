...
<span class="move2cart">Me déplacer au panier</span>
<?php } ?>
<?php */ ?>
</div>
...
<?php if ($v['avail'] > 0) {?>
<button id="pid<?php echo $v['productid'];?>">Ajouter au panier</button>
<?php } else  { ?>
...
<?php } ?>
<br />
<a class="main-button" onclick="javascript: return <?php if ($login) {?>add_wishlist(<?php echo $v['productid'];?>);<?php } else  { ?>login_popup();<?php } ?>" href="javascript: void(0);">Ajouter à la liste d'envies</a>
 </div>
</div>
<?php } ?>
</div>
