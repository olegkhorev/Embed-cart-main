	    ...
	    $ec.ajax({
           ...
           success: function(data) {
						...
						if (data == 'C') {
							...
							ec_alert("Votre lettre a été envoyée. Nous prendrons contact avec vous dès que possible.");
						}
//						location.reload();
           }
         });
		    ...
		    $ec.ajax({
    	       ...
    	       success: function(data)
        	   {
					if (data == 'Error') {
						...
						ec_alert('Veuillez patienter, nous connectez-vous à PayPal');
					} else {
							$ec('.invoice_quick_link').remove();
							$ec('body').append('<a class="ec_ajax_link invoice_quick_link" href="'+parnet_site_page+'#!/invoice/'+data+'/success"></a>');
							ec_ajax_clicks();
...
//							self.location = parnet_site_page+'#!/invoice/'+data+'/success';
	...
	$ec('.apply_gc').on('click', function() {
		...
		$ec('.coupon_screen').html('<br/><div class="group"><input type="text" size="30" required /><span class="highlight"></span><span class="bar"></span><label>Введите ваш код здесь</label></div><div class="coupon-error"></div><button>Appliquer</button><br/><br/>');
	...
	$ec('.apply_coupon').on('click', function() {
		bc = false;
		var id = ec_alert('<div class="coupon_screen"></div>', 1, '', 'coupon_alert');
		$ec('.coupon_screen').html('<br/><div class="group"><input type="text" size="30" required /><span class="highlight"></span><span class="bar"></span><label>Entrez le code de votre coupon ici</label></div><div class="coupon-error"></div><button>Appliquer</button><br/><br/>');
		$ec('.coupon_screen input').on('keyup', function() {
			$ec('.coupon-error').hide();
		});
...
function instant_search() {
	$ec('.search input').unbind('focus').on('focus', function() {
		var val = $ec(this).val();
		if (val.length < 2) {
			$ec('.instant-search').html("<div class='enter-3-chars'>Entrez 2 caractères</div>");
			return;
		}
