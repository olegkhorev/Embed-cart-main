...
function product_options(o) {
	...
	if ($ec(ec_qadd+'#hasVariants').size() > 0) {
		...
		if (variantid > 0) {
			...
			html += '<td nowrap><a href="javascript: void(0);" class="ec_clearAll">Effacer tous les</a></td>';
			...
			if (cnt == cnt2) {
				...
				bc = false;
				ec_alert("Cette option n'est pas disponible");
			} else {
...
function map_variants(o, groupid) {
	...
	$ec(ec_qadd+'.unavailable').each(function() {
		if (!$ec(this).parent().hasClass('novar')) {
			...
			$ec(ec_qadd+'.options_container img.checked').each(function() {
				if (id != $ec(this).parent().parent().attr('id')) {
					html += '<tr><td>'+$ec(this).parent().attr('data-title')+'</td><td class="ec_clear"><a href="javascript: void(0);" class="ec_clear-option" id="co-'+$ec(this).parent().parent().attr('id').replace('pog-', '')+'">Claire</a></td></tr>';
					found = true;
				}
			});
			...
			$ec(ec_qadd+'.product_options').each(function() {
				if (id != $ec(this).attr('id').replace('po-', '')) {
					html += '<tr><td>'+$ec(this).find('option:selected').text()+'</td><td class="ec_clear"><a href="javascript: void(0);" class="ec_clear-option" id="co-'+$ec(this).attr('id').replace('po-', '')+'">Claire</a></td></tr>';
					found = true;
				}
			});
		}
		...
		html += '</table>';
		if (found)
			$ec(this).append("<div class='help-option'>Option Non Disponible en Combinaison avec"+html+'<img src="'+current_location+'/images/spacer.gif" alt="" /></div>');
		else
			$ec(this).append("<div class='help-option'>Option Non Disponible<img src='"+current_location+"/images/spacer.gif' alt='' /></div>");
	});
...
//		alert(is_quick+'|'+ec_qadd);
		unload();
		if (r == "1") {
			ec_alert("S'il vous plaît, sélectionnez options");
			return false;
		}

		if (mobile_design) {
			ec_alert("Produit a été ajouté à votre panier.<br /><br /><a href='/cart'><button>Voir le panier</button></a> &nbsp; <a href='/checkout'><button>La caisse</button></a>", 1);
		}
...
function add_review() {
	...
	$ec.ajax({
		...
		unload();
		if (r == "1") {
			ec_alert("S'il vous plaît, entrez votre nom et votre message");
			return false;
		} else if (r == '2') {
			recaptchaOnload();
			ec_alert("Captcha est incorrect");
			return false;
		} else if (r == '3') {
			recaptchaOnload();
			ec_alert("Votre commentaire a été envoyé à la modération");
...
function add_wishlist(id) {
	$ec.ajax({
		...
		ec_alert("Produit a été ajouté à votre liste de Souhaits");
	});
}

function buy_one_click(pid) {
	ec_alert("Nous vous contacterons bientôt<br /><input type='text' placeholder='"+lbl_buy1click+"' /> <a class='main-button'>Envoyer</a>", 1, '', 'buy_one_click');
	$ec('.buy_one_click a.main-button').on('click', function() {
		...
		$ec.ajax({
			...
			if (r == 'N') {
				ec_alert("Vous pouvez exiger vente rapide, pas plus d'une fois toutes les 10 secondes");
			} else {
				ec_alert("S'il vous plaît, donnez-nous quelques minutes pour recevoir votre demande");
			}
		});
