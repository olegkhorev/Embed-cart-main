...
product_avail[<?php echo $oid;?>] = <?php echo $product['avail'];?>;
 ...
 <td class="details">
<?php if ($oid == 1) {?> <div class="see_all"><a class="ec_ajax_link" href="<?php echo $parnet_site_page;?>#!<?php echo $product['cleanurl'] ? $product['cleanurl'].'.html' : 'product/'.$product['productid'];; ?>">See more details</a></div>
<?php } ?>
...
<?php } ?>
 <td class="name"<?php if (!$average_rating) {?> style="padding-right: 15px; width: 130px; text-align: right;"<?php } ?>>SKU:</td>
...
<tr>
 ...
 <td class="value product-list-price"><s><?php echo '<span class="currency">'.currency_symbol().'</span>'.price_format_currency($product['list_price']); ?></s>, discount <?php echo '<span class="currency">'.currency_symbol().'</span>'.price_format_currency($save_price); ?> (<?php echo $save_percentage;?>%)</td>
</tr>
<?php } ?>
...
<?php if (false && $option_groups) {?><h3 class="visible">Product options</h3>
<?php } ?>
...
<tr>
 ...
 <td class="product-option"><strong class="subtitle"><?php echo $v['fullname'] ? $v['fullname'] : $v['name'];; ?><span id="pot-<?php echo $v['groupid'];?>"></span> <a href="javascript: void(0);" class="ec_clear_option" id="poa-<?php echo $v['groupid'];?>">Clear</a></strong><br />
...
<tr class="options-error">
 <td colspan="4" align="left">Please, select all options.</td>
</tr>
...
<?php } ?>
&nbsp; &nbsp;
<a class="main-button add2wl-pd add2wl-pd-desktop" onclick="javascript: return <?php if ($login) {?>add_wishlist(<?php echo $product['productid'];?>);<?php } else  { ?>ec_alert('Please login first');<?php } ?>" href="javascript: void(0);">Add to wishlist</a>
<br /><br />
 </td>
</tr>
...
<input type="hidden" name="options_ex" value="" />
 <button class="add2cart add2cart-product" id="productid-<?php echo $product['productid'];?>"><span><img src="<?php echo $current_location;?>/images/spacer.gif" alt="Добавить в корзину" /></span>Add to cart</button>
 </td>
<?php } else  { ?>
 <td class="out-of-stock" colspan="4"> &nbsp; Out of stock</td>
<?php } ?>

</tr>
<tr>
 <td colspan="4">
<a class="main-button add2wl-pd add2wl-pd-mobile" onclick="javascript: return <?php if ($login) {?>add_wishlist(<?php echo $product['productid'];?>);<?php } else  { ?>ec_alert('Please login first');<?php } ?>" href="javascript: void(0);">Add to wishlist</a>
...
<?php if ($product['brand_image']) {?>
<a href="<?php echo $parnet_site_page;?>#!brands/<?php echo $product['brand']['cleanurl'];?>" class="ajax_link">
<?php 
...
<?php } ?>
<a href="<?php echo $parnet_site_page;?>#!brands/<?php echo $product['brand']['cleanurl'];?>" class="ajax_link"><?php echo $product['brand']['name'];?></a>
</div>
<?php } ?>
<a class="main-button buy-with-one-click" onclick="javascript: buy_one_click(<?php echo $product['productid'];?>);" href="javascript: void(0);">Buy with one click</a>
<br /><br />
<a href="<?php echo $parnet_site_page;?>#!help?subject=<?php echo escape($product['name']); ?>" class="ajax_link link-button">Ask a question about this product</a>
<br /><br /><br />
...
<ul class="ec_product-tabs">
 <li class="tab-1 active" data-tab="1">Description</li>
<?php if ($related_products) {?>
 <li class="tab-2" data-tab="2">Related products</li>
<?php } ?>
<?php /* ?>
<?php if ($recommends) {?>
 <li class="tab-3" data-tab="3">Recommended products</li>
<?php } ?>
<?php */ ?>
 <li class="tab-4" data-tab="4">Send to friend</li>
 <li class="tab-5" data-tab="5">Reviews</li>
</ul>
</div>
...
<h3 class="its4mobile">Send to friend</h3>
...
<input type="hidden" name="mode" value="send_to_friend" />
  ...
  <table>
    ...
    <tr>
      <td colspan="2"></td>
      <td id="send_message_box"><label><input type="checkbox" id="is_msg" name="is_msg" value="1" onclick="javascript: $ec('.message-box').toggle();" value="Y"<?php if ($send_to_friend['is_msg']) {?> checked<?php } ?> />Add personal message</label>
...
<h3 class="its4mobile">Reviews</h3>
<?php if ($reviews) {?>
