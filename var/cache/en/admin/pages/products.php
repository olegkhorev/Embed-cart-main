...
<tr>
 <td class="data-name">Search by substring:</td>
...
<tr>
 <td colspan="2"><a href="javascript: void(0);" onclick="$ec(this).hide(); $ec('#advanced_search').show();">Advanced search</a></td>
</tr>
</table>
...
?>
<tr>
 <td class="data-name">Search by brand:</td>
...
<tr>
 <td class="data-name">Quantity:</td>
...
<tr>
 <td class="data-name">Weight (lbs):</td>
...
<tr>
 <td class="data-name">Availability:</td>
 <td><select name="status">
 <option value=""></option>
 <option value="1"<?php  if ($search_products['status'] == 1) echo ' selected="selected"'; ?>>Available for sale</option>
 <option value="2"<?php  if ($search_products['status'] == 2) echo ' selected="selected"'; ?>>Not available</option>
 <option value="3"<?php  if ($search_products['status'] == 3) echo ' selected="selected"'; ?>>Hidden, but available for sale</option>
 </select></td>
</tr>
<tr>
 <td></td>
 <td><br /><button>Search</button></td>
</tr>
</table>
<br /><br />
<?php 
...
if ($total_pages > 2) {
?>
<?php include SITE_ROOT."/var/cache/en/common/navigation_admin.php";?>
<br />
<?php 
}
?>
...
<tr>
 <th width="10"></th>
 <th><a href="<?php echo $current_location;?>/admin/products?sort=sku<?php  echo $direction; ?>">SKU</a></th>
 <th width="100%"><a href="<?php echo $current_location;?>/admin/products?sort=name&direction=<?php if ($_GET['direction']) {?>0<?php } else  { ?>1<?php } ?>">Product name</a></th>
 <th><a href="<?php echo $current_location;?>/admin/products?sort=price<?php  echo $direction; ?>">Price</a></th>
 <th><a href="<?php echo $current_location;?>/admin/products?sort=avail<?php  echo $direction; ?>">In stock</a></th>
 <th>Status</th>
</tr>
<?php 
...
<select name="posted_data['.$v['productid'].'][status]">
 ...
 <option value="3"'.($v['status'] == 3 ? ' selected' : '').'>Hidden, but available for sale</option>
</select>
 </td>
</tr>
		';	}
...
<button type="button" onclick="submitForm(this, 'update');">Save</button>
&nbsp; <button type="button" onclick="export_selected();">Export selected</button>
&nbsp;
<button type="button" onclick="if (confirm('Are you sure?', $ec(this)) || confirmed) submitForm(this, 'delete_products');">Delete selected</button>
</div>
