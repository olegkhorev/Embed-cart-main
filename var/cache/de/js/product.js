...
function product_options(o) {
	...
	if ($ec(ec_qadd+'#hasVariants').size() > 0) {
		...
		if (variantid > 0) {
			...
			html += '<td nowrap><a href="javascript: void(0);" class="ec_clearAll">Deaktivieren Sie alle</a></td>';
			...
			if (cnt == cnt2) {
				...
				bc = false;
				ec_alert("Diese Optionen Kombination ist nicht verfügbar");
			} else {
...
function map_variants(o, groupid) {
	...
	$ec(ec_qadd+'.unavailable').each(function() {
		if (!$ec(this).parent().hasClass('novar')) {
			...
			$ec(ec_qadd+'.options_container img.checked').each(function() {
				if (id != $ec(this).parent().parent().attr('id')) {
					html += '<tr><td>'+$ec(this).parent().attr('data-title')+'</td><td class="ec_clear"><a href="javascript: void(0);" class="ec_clear-option" id="co-'+$ec(this).parent().parent().attr('id').replace('pog-', '')+'">Klar</a></td></tr>';
					found = true;
				}
			});
			...
			$ec(ec_qadd+'.product_options').each(function() {
				if (id != $ec(this).attr('id').replace('po-', '')) {
					html += '<tr><td>'+$ec(this).find('option:selected').text()+'</td><td class="ec_clear"><a href="javascript: void(0);" class="ec_clear-option" id="co-'+$ec(this).attr('id').replace('po-', '')+'">Klar</a></td></tr>';
					found = true;
				}
			});
		}
		...
		html += '</table>';
		if (found)
			$ec(this).append("<div class='help-option'>Option Nicht Verfügbar in Kombination mit"+html+'<img src="'+current_location+'/images/spacer.gif" alt="" /></div>');
		else
			$ec(this).append("<div class='help-option'>Option Nicht Verfügbar<img src='"+current_location+"/images/spacer.gif' alt='' /></div>");
	});
...
//		alert(is_quick+'|'+ec_qadd);
		unload();
		if (r == "1") {
			ec_alert("Bitte, wählen Sie Optionen");
			return false;
		}

		if (mobile_design) {
			ec_alert("Produkt wurde zu Ihrem Warenkorb Hinzugefügt.<br /><br /><a href='/cart'><button>Warenkorb anzeigen</button></a> &nbsp; <a href='/checkout'><button>Kasse</button></a>", 1);
		}
...
function add_review() {
	...
	$ec.ajax({
		...
		unload();
		if (r == "1") {
			ec_alert("Bitte geben Sie Ihren Namen und Ihre Nachricht");
			return false;
		} else if (r == '2') {
			recaptchaOnload();
			...
			recaptchaOnload();
			ec_alert("Ihre Bewertung wurde an die moderation");
...
function add_wishlist(id) {
	$ec.ajax({
		...
		ec_alert("Produkt wurde zu Ihrem Wunschzettel Hinzugefügt");
	});
}

function buy_one_click(pid) {
	ec_alert("Wir werden Sie umgehend Kontaktieren<br /><input type='text' placeholder='"+lbl_buy1click+"' /> <a class='main-button'>Senden</a>", 1, '', 'buy_one_click');
	$ec('.buy_one_click a.main-button').on('click', function() {
		...
		$ec.ajax({
			...
			if (r == 'N') {
				ec_alert("Sie benötigen schnellen Verkauf nicht mehr als einmal pro 10 Sekunden");
			} else {
				ec_alert("Bitte erlauben Sie uns ein paar Minuten, um Ihre Anfrage erhalten");
			}
		});
