...
function submitBrand() {
	...
	if (document.brandform.name.value == '') {
		...
		ec_alert("Brand-name kann nicht leer sein");
...
function submit_category() {
	...
	if (!document.category_form.title.value) {
		...
		ec_alert("Bitte geben Sie Kategorie, Titel");
...
function remove_wp(id) {
	...
	if ($ec('#wp_tr-'+id+' .removed .help').size() > 0) {
		...
		$ec('#wp_tr-'+id+' .removed').append('<div class="help">Klicken Sie auf die <b>Update</b><br /> - Taste, um<br />entfernen</div>');
