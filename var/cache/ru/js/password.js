var pw_form = [];
pw_form['new_pswd'] = "Новый пароль";
pw_form['con_pswd'] = "Подтвердите пароль";
			...
			b.on('click', function() {
				...
				for (var x in pw_form) {
					...
					if (v == pw_form[x] || v == '') {
						...
						ec_alert("Пожалуйста, введите "+pw_form[x]);
				...
				if ($ec('input[name=new_pswd]').val() != $ec('input[name=con_pswd]').val()) {
					...
					ec_alert("Пожалуйста, введите в поле правильный пароль для подтверждения");
