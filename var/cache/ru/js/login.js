...
function restore_password() {
	...
	ec_alert("Ваш E-mail: <input placeholder=\"Сначала введите Ваш email \" type='text'><div></div>", 1, '', 'res_p');
	...
	$ec('.res_p input').on('keyup', function() {
		$ec.ajax({url: current_location+'/res_p?e='+$ec(this).val()+'&its_ajax_page=1'}).done(function(r) {
			...
			$ec('.res_p button').on('click', function() {
				...
				ec_alert("Ссылка для смены пароля была отправлена на ваш email");
			});
		});
	});
