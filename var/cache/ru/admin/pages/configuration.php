<form method="POST">
<?php if ($get['2'] == 'General') {?>
<h1>General settings</h1>
<?php } else if ($get['2'] == 'Company') {?>
<h1>Company information</h1>
<?php } else if ($get['2'] == 'Blog') {?>
<h1>Blog settings</h1>
<?php } ?>
...
foreach ($conf as $v) {
	if ($v['type'] == 'separator')
		echo '<tr><td colspan="2"><h3>'.lng($v['comment']).'</h3></td></tr>';
	else  {
		echo '<tr><td class="name">'.lng($v['comment']).'</td><td class="value">';
		...
		if ($v['type'] == 'text' || $v['type'] == 'numeric') {
			...
			echo '</select>';
		} else if ($v['name'] == 'default_country') {
...
<?php 
		...
		} else if ($v['name'] == 'location_country') {
