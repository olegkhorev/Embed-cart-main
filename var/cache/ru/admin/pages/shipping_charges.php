...
<b>Редактирование тарифов для</b><br />

<select name="shippingid" onchange="document.zoneform.submit()">
  <option value="">Всех способов</option>
<?php 
foreach ($shipping as $s)
	echo '<option value="'.$s['shippingid'].'"'.(($_GET['shippingid'] != "" && $_GET['shippingid'] == $s['shippingid']) ? ' selected="selected"' : '').'>'.func_trademark($s['shipping']).' ('.($s['destination'] == "I" ? "Международная" : "Национальная").')</option>';
?>
</select>

<select name="zoneid" onchange="document.zoneform.submit()">
  <option value="">Всех зон</option>
<?php 
foreach ($zones as $z)
...
?>
<div align="right"><a href="#addrate">Добавить стоимость тарифов доставки</a></div>
...
<tr>
  <th><input type="checkbox" id="sm_'.$z['zone']['zoneid'].'_'.$shipid.'" name="sm_'.$z['zone']['zoneid'].'_'.$shipid.'" onchange="javascript: bc = false; if ($ec(this).is(\':checked\')) { $ec(\'.checkboxes_'.$z['zone']['zoneid'].'_'.$shipid.'\')[0].checked = true; }else  $ec(\'.checkboxes_'.$z['zone']['zoneid'].'_'.$shipid.'\')[0].checked = false; " /></th>
  <th><b><label for="sm_'.$z['zone']['zoneid'].'_'.$shipid.'">'.func_trademark($shipping_method['shipping']).' ('.($shipping_method['destination'] == "I" ? "Международная" : "Национальная").')</label></b></th>
  <th align="right">
  </th>
</tr>
</table>
...
<input type="text" name="posted_data['.$shipping_rate['rateid'].'][maxtotal]" size="9" value="'.$shipping_rate['maxtotal'].'" />
  ...
  <td>Взимание платы за единицу товара ('.$config['General']['currency_symbol'].'):</td>
...
if ($type == "D") {
?>
<h3>Добавить стоимость доставки</h3>
<?php 
} else  {
?>
<h3>Добавить надбавки к доставке</h3>
<?php 
}
...
<tr>
  <td width="150"><b>Способ доставки :</b></td>
  <td>
  <select name="shippingid_new">
    <option value="">Выберите один</option>
<?php 
	foreach ($shipping as $s)
		echo '<option value="'.$s['shippingid'].'">'.func_trademark($s['shipping']).' ('.($s['destination'] == "I" ? "Международная" : "Национальная").')</option>';
?>
  </select>
  </td>
</tr>
...
<?php /* ?>
<tr>
  <td><b>Применить тариф к:</b></td>
  <td>
  <select name="apply_to_new">
    <option value="DST" selected="selected">DST (Итого со скидкой)</option>
    <option value="ST">ST (Итого)</option>
  </select>
  </td>
</tr>
<?php */ ?>
...
<input type="text" name="maxweight_new" size="9" value="<?php  echo price_format($maxvalue); ?>" />
  ...
  <td><b>Фиксированной цене (<?php  echo $config['General']['currency_symbol']; ?>):</b></td>
  <td nowrap="nowrap"><input type="text" name="rate_new" size="5" value="0.00" /></td>
  <td><b>Процентному сбору:</b></td>
  <td><input type="text" name="rate_p_new" size="5" value="0.00" /></td>
</tr>

<tr>
  <td width="150"><b>Промежуточному итогу:</b></td>
...
<input type="text" name="maxtotal_new" size="9" value="<?php  echo price_format($maxvalue); ?>" />
  ...
  <td><b>Взимание платы за единицу товара (<?php  echo $config['General']['currency_symbol']; ?>):</b></td>
...
?>
Установленные пользователем способы доставки не определяются
<?php 
}
?>
