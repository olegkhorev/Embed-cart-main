...
<tr>
  ...
  <td align="center"><select name="data['.$s['shippingid'].'][destination]">
    <option value="I"'.($s['destination'] == 'I' ? ' selected="selected"' : '').'>Международная</option>
    <option value="N"'.($s['destination'] == 'N' ? ' selected="selected"' : '').'>Национальная</option>
  </select></td>
  ...
  <td><button type="button" onclick="self.location=\''.$current_location.'/admin/shipping/?mode=delete&amp;shippingid='.$s['shippingid'].'\'">Удалить</button></td>
</tr>';
	}
}
?>
...
<tr>
  <td colspan="7"><br /><h3>Добавить способ доставки </td>
</tr>
...
<tr>
  ...
  <td align="center"><select name="add[destination]">
    <option value="I">Международная</option>
    <option value="N">Национальная</option>
  </select></td>
