...
extract($_GET);
if (php_sapi_name() != 'cli' && $pswd != 'embed_01230') {
	header("Location: /");
	exit;
}
	...
	function parse_js($lng, $file) {
		...
		if (!empty($matches['1']['0'])) {
			foreach ($matches['0'] as $k2=>$file2) {
				$to = $matches['1'][$k2];
				$tmp = $db->field("SELECT translation FROM languages WHERE lng='".$lng."' AND word='".$to."'");
				if (empty($tmp)) {
		...
		$packed = $script;
		if ($file == 'jquery.min') {
#			$packed = '/*! jQuery v1.12.4 | (c) jQuery Foundation | jquery.org/license */
#'.$packed;
 ...
 */'.$packed;
	...
	foreach ($templates['css'] as $file=>$v) {
		$file = file(SITE_ROOT.'/templates/css/'.$file.'.css');
		if ($file)
		...
		$content = str_replace("\t", "", $content);
		$content = str_replace(";}", "}", $content);
		$content = str_replace("url('", "url('".$web_dir."", $content);
