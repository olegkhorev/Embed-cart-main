...
<form method="post" name="zoneform" onsubmit="javascript: if (this.zone_name.value == '') { ec_alert('{lng[Zone name must be entered]}'); return false; } else return saveSelects(new Array('zone_countries','zone_ec_states'));"{*{if !$zone['zoneid']} class="noajax"{/if}*}>
...
<br /><br />
<button type="submit"><?php if ($zone['zone_name']) echo "{lng[Update|escape]}"; else echo "{lng[Create|escape]}"; ?></button>
<?php
if ($zoneid) {
?>
&nbsp;&nbsp;
...
<tr>
  <td>
<input type="hidden" id="zone_ec_states_store" name="zone_ec_states_store" value="" />
<select id="zone_ec_states" multiple="multiple" class="width-100p" size="20">
<?php
foreach ($zone_ec_states as $s)
	echo '<option value="'.$s['country_code'].'_'.$s['code'].'">'.substr($s['country'], 0, 30).': '.$s['state'].'</option>';
?>
<option value="">&nbsp;</option>
</select>
<script type="text/javascript">
normalizeSelect('zone_ec_states');
</script>
  </td>
  <td align="center">
<input type="button" value="&lt;&lt;" onclick="javascript: moveSelect(document.getElementById('zone_ec_states'), document.getElementById('rest_ec_states'), 'R');" />
<br /><br />
<input type="button" value="&gt;&gt;" onclick="javascript: moveSelect(document.getElementById('zone_ec_states'), document.getElementById('rest_ec_states'), 'L');" />
  </td>
  <td>
<select id="rest_ec_states" name="rest_ec_states" multiple="multiple" class="width-100p" size="20">
<?php
foreach ($rest_ec_states as $s)
