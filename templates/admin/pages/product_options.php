...
<tr>
 ...
 <td nowrap>';
	...
	if ($v['view_type'] == 's') {
 		echo "{lng[Select box|escape]}";
	} elseif ($v['view_type'] == 'p') {
 		echo "{lng[Squares|escape]}";
	} elseif ($v['view_type'] == 'r') {
 		echo "{lng[Radio buttons list|escape]}";
	} elseif ($v['view_type'] == 't') {
 		echo "{lng[Text area|escape]}";
	} else {
 		echo "{lng[Input box|escape]}";
 	}
...
?>
<tr>
 <td colspan="6"><br><button type="submit">{lng[Update]}</button> <button type="button" onclick="submitForm(this, 'delete');">{lng[Delete selected]}</button></td>
</tr>
<?php
}
?>
<tr>
 <td colspan="6"><br /><a href="{$current_location}/admin/products/<?php echo $get['2']; ?>/options/add">{lng[Add new]}</a></td>
</tr>
</table>
<?php
