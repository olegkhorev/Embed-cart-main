...
?>
<h3>{lng[Products]}</h3>
<form method="POST" name="psform" href="{$current_location}/admin/products">
...
<tr>
 <td colspan="2"><a href="javascript: void(0);" onclick="$ec(this).hide(); $ec('#advanced_search').show();">{lng[Advanced search]}</a></td>
</tr>
</table>
...
if ($total_pages > 2) {
?>
{include="common/navigation_admin.php"}
<br />
<?php
}
?>
...
<tr>
 <th width="10"></th>
 <th><a href="{$current_location}/admin/products?sort=sku<?php echo $direction; ?>">{lng[SKU]}</a></th>
 <th width="100%"><a href="{$current_location}/admin/products?sort=name&direction={if $_GET['direction']}0{else}1{/if}">{lng[Product name]}</a></th>
 <th><a href="{$current_location}/admin/products?sort=price<?php echo $direction; ?>">{lng[Price]}</a></th>
 <th><a href="{$current_location}/admin/products?sort=avail<?php echo $direction; ?>">{lng[In stock]}</a></th>
 <th>{lng[Status]}</th>
</tr>
<?php
...
&nbsp;
<button type="button" onclick="if (confirm('Are you sure?', $ec(this)) || confirmed) submitForm(this, 'delete_products');">{lng[Delete selected]}</button>
</div>
