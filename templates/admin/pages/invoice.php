...
<tr>
 <td width="50%">
{if $order['packages_result']}
<h1>Коробки</h1>
{foreach $order['packages_result'] as $k=>$pack}
Коробка №{php echo $k+1}<br />
{foreach $pack as $prod}
{$prod['name']} - {$prod['qty']}<br />
{/foreach}
<hr />
{/foreach}
{/if}
...
<button onclick="javascript: self.location='/admin/invoice/<?php echo $order['orderid']; ?>/status/'+$ec('#status').val();">{lng[Save]}</button>
)</h1>
...
Payment Intent: {$order['payment_indent']}
<br />
{/if}
<br />
{/if}
