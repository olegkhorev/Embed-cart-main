...
<form method="POST" href="{$current_location}/admin/reviews?mode=search">
...
<form action="{$current_location}/admin/reviews" method="post" name="reviewsform">
...
{foreach $reviews as $r}
                <tr>
                        ...
                        <td valign="top" width="10%"><a href="{$current_location}/admin/products/{$r['productid']}" target="_blank">{$r['productid']}/{$r['sku']}</a></td>
                </tr>
{/foreach}
                <tr>
                        <td colspan="9"><br />
...
<button type="button" onclick="javascript: if (confirmed || confirm('{lng[This operation will delete selected reviews.|escape]}', $ec(this))) submitForm(this, 'delete');">{lng[Delete selected]}</button>
</div>
                        </td>
                </tr>
        </table>
