...
if ($category) {
	if ($category['enabled'] != 1)
		echo "<font color='#c60'>{lng[Category disabled|escape]}</font><br /><br />";

	echo '<a href="'.$current_location.'/admin/category/'.$category['categoryid'].'" class="ec_ajax_link">'."{lng[Modify category|escape]}".'</a> | ';
	echo '<a href="'.$current_location.'/admin/category/'.$category['categoryid'].'/products" class="ec_ajax_link">'."{lng[Category products|escape]}".'</a> | ';
	echo '<a href="javascript: void(0);" onclick="javascript: confirm(\'\', $ec(\'#delete_category\'), \'/admin/category/'.escape($category['categoryid'], 3).'/delete\'); return false;">'."{lng[Delete category|escape]}".'</a><br /><br />';
?>
...
<?php
  ...
  <td><a href="'.$current_location.'/admin/categories/'.$v['categoryid'].'">'.$v['title'].'</a> (<a href="'.$current_location.'/admin/category/'.$v['categoryid'].'">'."{lng[Edit|escape]}".'</a>)</td>
...
?>
<button type="button" onclick="self.location='{$current_location}/admin/category?parentid=<?php echo $category['categoryid']; ?>'">{lng[Add new category]}</button>
</form>
