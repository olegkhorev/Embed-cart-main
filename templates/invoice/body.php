...
<a href="{$current_location}/invoice/{$order['orderid']}/print" target="_blank" onclick="javascript: return print_invoice($ec(this));">{lng[Print invoice]}</a>
<a href="{$current_location}/invoice_pdf/{$order['orderid']}" target="_blank">{lng[Download PDF invoice]}</a>
<br /><br />
</div>{/if}
{/if}
<div style="width: 650px; margin: 0 auto; background: #fff; padding: 15px 10px; {if $get['2'] != 'print' && !$is_mail}border: 1px solid #ccc; border-radius: 16px;{/if}" class="invoice">
...
Payment method: {$order['payment_method']}{if $order['payment_details']} ({$order['payment_details']}){/if}
{/if}
{if $order['tracking']}
<br />
...
<a href="{$current_location}" target="_blank">{$current_location}</a><br /><br />
...
{/if}
	<tr>
	 <td class="image hideonmobile" valign="top" style="padding-right: 10px;"><a href="{$parent_site_location}{$parnet_site_page}#!/{$url}" class="ec_ajax_link" style="border-radius: 16px; position: relative; overflow: hidden; display: inline-block;">
{if $v['variant_photo']}
<?php
...
{/if}
{else}
<a href="{$parent_site_location}{$parnet_site_page}#!/{$url}" class="ec_ajax_link">{$v['name']}</a>
{*
{if $v['weight']}
<br />
