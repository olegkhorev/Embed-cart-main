<h1 class="ec_cart-h1">{lng[Your cart]}</h1>
{if $products}
<form action="{$current_location}/cart" id="cartform" method="POST">
...
{/if}
	...
	<td><a href="{$current_location}/cart/remove/{$v['cartid']}?its_ajax_page=1" class="remove-link remove-cart-link"><img src="{$current_location}/images/delete.png" alt="" /></a></td>
	</tr>
{/foreach}
</tbody>
</table>
...
<table width="100%">
<tr>
 <td><button<?php if ($is_ajax) echo ' type="button"'; ?> class="update-cart">{lng[Update]}</button> &nbsp; <button type="button" class="ec_clear-cart main-button"<?php if (!$is_ajax) echo ' onclick="self.location=\'cart/clear\'"'; ?>>{lng[Clear cart]}</button></td>
...
{/if}
<div class="totals">
 <b>{lng[Total]}: {price $cart['total']}</b>
</div>
<br /><br />
<button class="checkout-link ec_ajax_link add2cart-product" href="{$parnet_site_page}#!checkout" type="button">{lng[Checkout]}</button></td>
</tr>
</table>
</form>
{else}<br /><br />
