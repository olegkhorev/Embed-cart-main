...
<div class="news-search"><input value="<?php echo $news_substring ? $news_substring : 'Search'; ?>" onfocus="javascript: if (this.value == 'Search') this.value = '';" onblur="javascript: if (this.value == '') this.value = 'Search';" type="text" name="substring" /><input type="image" src="{$current_location}/images/spacer.gif" alt="" /></div>
</form>
*}
<h1>News</h1>
...
<div class="back"><a href="/news" class="ec_ajax_link">{lng[Back to News mainpage]}</a></div>
</div>
...
<a class="ec_ajax_link" href="{$url}"><h5>{$b['title']} (<span class="date"><?php echo date($date_format, $b['date']); ?></span>)</h5></a>
<div class="news-item">
<?php
	if ($b['imageid']) {
		echo '<a class="ec_ajax_link" href="'.$url.'">';
		$image = $b;
		$image['new_width'] = 250;
...
<br />
<a class="news-more ec_ajax_link" href="{$url}">{lng[See full article]}</a>
</div>
