...
function ec_ajax_clicks_admin() {
	...
	$ec('#data_type').change(function() {
		if ($ec(this).val() == 'g') {
			$ec('#data_view_type').html("<option value='s'>{lng[Select box]}</option><option value='p'>{lng[Squares]}</option>");
		} else {
			$ec('#data_view_type').html("<option value='t'>{lng[Text area]}</option><option value='i'>{lng[Input box]}</option>");
		}
	});
...
function submitBrand() {
	if (document.brandform.name.value == '') {
		...
		bc = false;
		ec_alert("{lng[Brand name cannot be empty]}");
	} else
...
function submit_category() {
	if (!document.category_form.title.value) {
		ec_alert("{lng[Please, enter category title]}");
		bc = false;
...
function remove_wp(id) {
	...
	if ($ec('#wp_tr-'+id+' .removed .help').size() > 0) {
		...
		$ec('#wp_tr-'+id+' .removed').append('<div class="help">{lng[Click the <b>Update</b><br />button to complete<br />removing]}</div>');
	}
