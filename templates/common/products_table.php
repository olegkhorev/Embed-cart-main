...
<tr class="ec_photos">
{foreach $row as $k=>$v}
<td><div><a class="ec_ajax_link" title="Move product to cart to add it" href="{$current_location}/{if $v['cleanurl']}{$v['cleanurl']}.html{else}product/{$v['productid']}{/if}">
{if $v['photo']}
<?php
$image = $v['photo'];
$image['new_width'] = 234;
...
<tr>
{foreach $row as $k=>$v}
 <td><a class="ec_ajax_link" href="{$current_location}/{if $v['cleanurl']}{$v['cleanurl']}.html{else}product/{$v['productid']}{/if}">{$v['name']}</a></td>
{/foreach}
{if $k == 0}
 <td colspan="3" width="100%"></td>
{elseif $k == 1}
...
{foreach $row as $k=>$v}
<td>
<a class="main-button ec_ajax_link" href="{$current_location}/{if $v['cleanurl']}{$v['cleanurl']}.html{else}product/{$v['productid']}{/if}">{lng[View]}</a>
{if $v['avail'] > 0}
