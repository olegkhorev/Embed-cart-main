...
<h1><a href="{$parnet_site_page}#!cart" class="ec_ajax_link back-to-cart"><img src="{$current_location}/images/back_arrow.png" alt="" /></a>{lng[Checkout]}</h1>
...
{php $url = $parnet_site_page.'#!'.($v['cleanurl'] ? $v['cleanurl'].'.html' : 'product/'.$v['productid']);}
	<tr>
	 <td class="image"><a class="ec_ajax_link" href="{$url}">
{if $v['variant_photo']}
<?php
...
<b>Gift Card</b>
{else}
<a class="ec_ajax_link" href="{$url}">{$v['name']}</a>
{/if}
{if $v['weight']}
<br />
...
{lng[You can proceed anonymous or]} <a class="ec_ajax_link" href="{$parnet_site_page}#!login">{lng[Login]}</a>, <a class="ec_ajax_link" href="{$parnet_site_page}#!register">{lng[Register]}</a>.
</div>
...
<form method="post" action="{$current_location}/checkout/user_form" id="checkout_user_form">
    <div class="group">
...
ec_states.{$v['code']}.ec_states[{$k}] = {code: "{$s['code']}", state: "{php echo escape($s['state'], 2);}"};
  {/foreach}
 {/if}
{/foreach}
</script>
...
{php $found = false;}
{foreach $countries as $v}
 {if $v['code'] == $userinfo['country'] && $v['ec_states']}
  {php $found = true;}
<select name="posted_data[state]" id="state">
  {foreach $v['ec_states'] as $s}
...
{php $found = false;}
{foreach $countries as $v}
 {if $v['code'] == $userinfo['b_country'] && $v['ec_states']}
  {php $found = true;}
...
<div align="center"><button class="add2cart-product">{lng[Continue]}</button></div>
</form>
</div>
 </td>
